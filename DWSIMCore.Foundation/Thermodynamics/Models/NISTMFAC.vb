...
Namespace PropertyPackages.Auxiliary
    ...
    <System.Serializable()> Public Class NISTMFAC
        ...
        Function GAMMA_MR(ByVal T As Double, ByVal Vx As Double(), ByVal VQ As Double(), ByVal VR As Double(), ByVal VEKI As List(Of Dictionary(Of Integer, Double))) As Double()
            ...
            Host.CheckAndAdd(IObj, "", "GAMMA_MR", "Modified UNIFAC (NIST) Activity Coefficient", "NIST-MODFAC Activity Coefficient Calculation Routine")
                            ...
                            If Not Me.ModfGroups.InteracParam_aij(g1).ContainsKey(g2) Then
                                If Me.ModfGroups.InteracParam_aij.ContainsKey(g2) Then
                                    If Not Me.ModfGroups.InteracParam_aij(g2).ContainsKey(g1) And g2 <> g1 Then
                                        Throw New Exception("NIST-MODFAC Error: Could not find interaction parameter for groups " & Me.ModfGroups.Groups(id1 + 1).GroupName & " / " &
        ...
        Function RET_VN(ByVal cp As ICompoundConstantProperties) As Dictionary(Of Integer, Double)
    ...
    <System.Serializable()> Public Class NistModfacGroups
        ...
        Sub New()
            ...
            Using filestr As IO.Stream = System.Reflection.Assembly.GetAssembly(Me.GetType).GetManifestResourceStream("DWSIMCore.Foundation.NIST-MODFAC_RiQi.txt")
            ...
            Using filestr As IO.Stream = System.Reflection.Assembly.GetAssembly(Me.GetType).GetManifestResourceStream("DWSIMCore.Foundation.NIST-MODFAC_IP.txt")
                Using parser As New TextFieldParser(filestr)
                    delimiter = vbTab
                    parser.SetDelimiters(delimiter)
                    fields = parser.ReadFields()
