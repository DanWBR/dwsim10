        ...
        Public Overrides Sub DW_CalcCompPartialVolume(ByVal phase As Phase, ByVal T As Double, ByVal P As Double)
            ...
            Select Case phase
                Case Phase.Liquid
                    ...
                    Next
                Case Phase.Aqueous
                    For Each subst As ICompound In Me.CurrentMaterialStream.Phases(6).Compounds.Values
                        subst.PartialVolume = subst.ConstantProperties.Molar_Weight / AUX_LIQDENSi(subst, T)
                    Next
                Case Phase.Liquid1
                    For Each subst As ICompound In Me.CurrentMaterialStream.Phases(3).Compounds.Values
                        subst.PartialVolume = subst.ConstantProperties.Molar_Weight / AUX_LIQDENSi(subst, T)
                    Next
                Case Phase.Liquid2
                    For Each subst As ICompound In Me.CurrentMaterialStream.Phases(4).Compounds.Values
                        subst.PartialVolume = subst.ConstantProperties.Molar_Weight / AUX_LIQDENSi(subst, T)
                    Next
                Case Phase.Liquid3
                    For Each subst As ICompound In Me.CurrentMaterialStream.Phases(5).Compounds.Values
                        subst.PartialVolume = subst.ConstantProperties.Molar_Weight / AUX_LIQDENSi(subst, T)
                    Next
                Case Phase.Vapor
