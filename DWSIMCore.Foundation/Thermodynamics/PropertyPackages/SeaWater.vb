...
Namespace PropertyPackages
    ...
    <System.Serializable()> Public Class SeawaterPropertyPackage
        ...
        Public Overrides Sub DW_CalcProp(ByVal [property] As String, ByVal phase As Phase)
            ...
            Dim salt As ICompound = (From subst As ICompound In Me.CurrentMaterialStream.Phases(0).Compounds.Values Select subst Where subst.ConstantProperties.Name = "Salt").SingleOrDefault
            ...
            If Not IgnoreSalinityLimit Then
                If salinity > Seawater.sal_smax Then
                    If Me.CurrentMaterialStream.Flowsheet IsNot Nothing Then
                        Me.CurrentMaterialStream.Flowsheet.ShowMessage(Me.ComponentName & "/" & New StackFrame(1).GetMethod.Name & "(): maximum salinity exceeded (" & Format(salinity, "0.00") & " kg/kg). Using upper limit value (" & Format(Seawater.sal_smax, "0.00") & " kg/kg).", IFlowsheet.MessageType.Warning)
                    End If
