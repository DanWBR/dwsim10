                ...
                For Each prop As PropertyInfo In props
                    ...
                    If Not skip Then
                        If prop.CanRead Then
                            ...
                            ElseIf TypeOf obj.GetType.GetProperty(prop.Name).GetValue(obj, Nothing) Is Single Then
                                .Add(New XElement(prop.Name, Single.Parse(obj.GetType.GetProperty(prop.Name).GetValue(obj, Nothing).ToString()).ToString("R", ci)))
                            ElseIf TypeOf obj.GetType.GetProperty(prop.Name).GetValue(obj, Nothing) Is Double Then
                                .Add(New XElement(prop.Name, Double.Parse(obj.GetType.GetProperty(prop.Name).GetValue(obj, Nothing).ToString()).ToString("R", ci)))
        ...
        If sourcearray.Count > 0 Then
            ...
            For Each obj As Object In sourcearray
                If TypeOf obj Is Double Then
                    sb += Double.Parse(obj.ToString()).ToString("R", ci) + ","
                Else
                    sb += obj.ToString + ","
                End If
            Next
