...
Public Class Host
    ...
    Public Shared Property CurrentSolutionID As String
        ...
        Set(value As String)
            _currentsid = value
            If Settings.ClearInspectorHistoryOnNewCalculationRequest Then
                Items.Clear()
                GC.Collect()
            End If
        End Set
    ...
    Public Shared Function GetNewInspectorItem(<CallerMemberName> Optional memberName As String = "", <CallerFilePath> Optional fileName As String = "", <CallerLineNumber> Optional lineNumber As Integer = 0) As InspectorItem
        ...
        If Settings.InspectorEnabled And Not Settings.ExcelMode And Not Settings.CAPEOPENMode Then
            Return New InspectorItem With {.CodePath = (fileName & "#L" & lineNumber).Replace(fileName.Substring(0, fileName.LastIndexOf("\dwsim\") + 7), "https://github.com/DanWBR/dwsim/blob/windows").Replace("\", "/")}
        Else
            Return Nothing
        End If
    ...
    Public Shared Sub CheckAndAdd(ii As InspectorItem, callingmethod As String, method As String, name As String, description As String, Optional current As Boolean = False)
        ...
        If ii IsNot Nothing Then
            ...
            Else
                ...
                Host.Items.Add(ii)
            End If
            If current Then Host.CurrentItem = ii
        End If
    End Sub
