...
- [NEW] The built-in PC-SAFT polymer set grows to fourteen, adding silicone, several acrylics, polybutadiene and poly(ethylene glycol), with parameters for many more solvents. Poly(ethylene glycol) is modelled as a hydrogen-bonding polymer. Any built-in polymer can be picked from the compound list by entering its molar mass, and the model is checked against published polymer cloud-point curves.
