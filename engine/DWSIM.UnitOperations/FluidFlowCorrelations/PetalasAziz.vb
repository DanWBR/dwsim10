...
Namespace FlowPackages
    ...
    <Serializable()> Public Class PetalasAziz
        ...
        'INPUT Variables
        Protected DensL As Single  'Liquid density (lb/ft3)
        Protected DensG As Single  'Gas density (lb/ft3)
        ...
        ''' <summary>
        ''' Managed replacement for the native PetAz.dll "calcpdrop" entry
        ''' point. The signature is kept so existing callers still compile;
        ''' the arguments carry the same units they always did.
        ''' </summary>
        Public Shared Sub calcpdrop(ByRef DensL As Single, ByRef DensG As Single, ByRef MuL As Single, ByRef MuG As Single,
            ...
            Dim solver As New PetalasAzizModel.PetalasAzizSolver()

            Dim r = solver.Calculate(DensL, DensG, MuL, MuG, Sigma, Dia, Rough, Theta, VsL, VsG)

            Region = CInt(r.Regime)
            dPfr = CSng(r.FrictionalGradient)
            dPhh = CSng(r.HydrostaticGradient)
            eL = CSng(r.LiquidHoldup)
        ...
        Public Overrides Function CalculateDeltaP(ByVal D As Double, ByVal L As Double, ByVal deltaz As Double, ByVal k As Double, ByVal qv As Double, ByVal ql As Double, ByVal muv As Double, ByVal mul As Double, ByVal rhov As Double, ByVal rhol As Double, ByVal surft As Double) As Object()
            ...
            Else
                ...
                calcpdrop(DensL, DensG, CSng(mul), CSng(muv), Sigma, Dia, Rough, Theta, VsL, VsG, Region, dPfr, dPhh, eL)
