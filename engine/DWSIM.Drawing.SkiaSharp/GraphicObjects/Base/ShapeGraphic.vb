...
Namespace GraphicObjects
    ...
    Public MustInherit Class ShapeGraphic
        ...
        Protected Overrides Sub Dispose(disposing As Boolean)
            ...
            Image?.Dispose()
            PhotoImage?.Dispose()
            AttentionImage?.Dispose()
            CalculatingImage?.Dispose()
            WeightImage?.Dispose()
            TemperatureImage?.Dispose()
            PressureImage?.Dispose()
            FlowImage?.Dispose()
            LightningImage?.Dispose()
            VolumeImage?.Dispose()
            ResidenceTimeImage?.Dispose()
        ...
        Friend Sub DrawPhoto(canvas As SKCanvas)

            If EmbeddedResourcePhotoName = "" Then
                DrawIcon(canvas)
                Return
            End If

            If PhotoImage Is Nothing Then

                If ImagePaint Is Nothing Then
                    ImagePaint = New SKPaint With {.IsAntialias = True, .FilterQuality = SKFilterQuality.High}
                End If

                Using us = Me.GetType().Assembly.GetManifestResourceStream(String.Format("DWSIM.Drawing.SkiaSharp.{0}", EmbeddedResourcePhotoName))
                    If us IsNot Nothing Then
                        Using bitmap = SKBitmap.Decode(us)
                            PhotoImage = SKImage.FromBitmap(bitmap)
                        End Using
                    End If
                End Using
            ...
            canvas.DrawImage(PhotoImage, New SKRect(X, Y, X + Width, Y + Height), ImagePaint)

            If Not Active Then
                Using p As New SKPaint With {.IsAntialias = False, .FilterQuality = SKFilterQuality.None}
                    p.BlendMode = SKBlendMode.Color
                    p.ColorFilter = SKColorFilter.CreateBlendMode(SKColors.Gray, SKBlendMode.SrcIn)
                    canvas.DrawImage(PhotoImage, New SKRect(X, Y, X + Width, Y + Height), p)
                End Using
            End If
        ...
        Public Overridable Sub DrawDynSpec(ByVal g As SKCanvas, ByVal DynSpec As Enums.Dynamics.DynamicsSpecType)
            ...
            Using tpaint As New SKPaint()
                ...
                Dim trect As New SKRect(0, 0, 2, 2)
                Using tp = tpaint.GetTextPath(text, 0, 0)
                    tp.GetBounds(trect)
                End Using
        ...
        Public Overridable Sub DrawTag(ByVal g As SKCanvas)
            ...
            Try
                ...
                If DrawLabel Then
                    ...
                    Dim trect As New SKRect(0, 0, 2, 2)
                    Using tp = tpaint.GetTextPath(Me.Tag, 0, 0)
                        tp.GetBounds(trect)
                    End Using
        ...
        Public Sub DrawReactor(ByVal g As SKCanvas, ByVal TypeName As String)
            ...
            If DrawMode <> 2 Then
                ...
                Dim trect As New SKRect(0, 0, 2, 2)
                Using tp = tPen.GetTextPath(TypeName, 0, 0)
                    tp.GetBounds(trect)
                End Using
