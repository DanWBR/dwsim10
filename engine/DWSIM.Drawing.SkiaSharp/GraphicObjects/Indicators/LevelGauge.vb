        ...
        Public Overrides Sub Draw(ByVal g As Object)
            ...
            If Not SelectedObject Is Nothing Then
                Using aPen As New SKPaint()
                    ...
                    Using path2 As New SKPath()
                        Dim points2() As SKPoint = PointList.Select(Function(x) New SKPoint(x.X, x.Y)).ToArray
                        path2.MoveTo(points2(0).X, points2(0).Y)
                        For i As Integer = 1 To points2.Length - 1
                            path2.LineTo(points2(i).X, points2(i).Y)
                        Next
                        canvas.DrawPath(path2, aPen)
                    End Using
                End Using
            End If
            ...
            Using paint2 As New SKPaint With {.Color = SKColors.Black, .IsStroke = False, .IsAntialias = True}
                Using triPath As New SKPath()
                    Dim tx = CSng(origin + w / 2 + 6 * f)
                    Dim ty = CSng(cly)
                    Dim ts = CSng(4 * f)
                    triPath.MoveTo(tx - ts, ty + ts)
                    triPath.LineTo(tx, ty - ts)
                    triPath.LineTo(tx + ts, ty + ts)
                    triPath.Close()
                    canvas.DrawPath(triPath, paint2)
                End Using
            End Using
