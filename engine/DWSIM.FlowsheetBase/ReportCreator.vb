...
Public Class ReportCreator
    ...
    ''' <summary>
    ''' Generates a self-contained, print-friendly HTML report for the whole flowsheet
    ''' (all objects, all phases). Delegates to the shared, UI-agnostic
    ''' <see cref="DWSIM.SharedClasses.Reports.SimulationReportBuilder"/> so the classic and
    ''' cross-platform UIs share one rendering path. The legacy ODT/ODS exporters below are
    ''' left untouched to preserve their exact output.
    ''' </summary>
    Public Function GenerateHTML(Optional options As DWSIM.SharedClasses.Reports.ReportOptions = Nothing) As String
        Return New DWSIM.SharedClasses.Reports.SimulationReportBuilder(frm).GenerateHTML(options)
    End Function
