...
Public Interface IFlowsheet
    ...
    ''' <summary>
    ''' Update the AI status label on the flowsheet. Pass isWorking=True with a
    ''' message to show a "thinking" animation, or isWorking=False / empty text
    ''' to clear it.
    ''' </summary>
    Sub UpdateAILabelStatus(text As String, isWorking As Boolean)
    ...
    Sub Connect(gobjfrom As ISimulationObject, gobjto As ISimulationObject, fromidx As Integer, toidx As Integer)

    Sub Disconnect(gobjfrom As ISimulationObject, gobjto As ISimulationObject)
    ...
    Property ErrorMessages As List(Of Exception)
    ...
    Property LastSuccessfulSolutionState As String

    Sub RestoreLastSuccessfulSolutionState()

    Sub SaveCurrentStateAsLastSuccessfulSolution()

    Sub SavePFDScreenshotToPNG(pngfilepath As String)

    ''' <summary>
    ''' Opens the data regression utility pre-loaded with the given binary
    ''' compound pair and model. Compounds and model are locked while open.
    ''' Returns the regressed parameters when the user clicks "Transfer", or
    ''' Nothing if the user cancels / closes without transferring.
    ''' </summary>
    Function CallDataRegressionUtility(comp1 As String, comp2 As String,
                                       model As String) As IInteractionParameter
