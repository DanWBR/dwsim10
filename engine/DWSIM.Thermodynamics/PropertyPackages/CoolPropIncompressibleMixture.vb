...
Namespace PropertyPackages
    ...
    <System.Serializable()> Public Partial Class CoolPropIncompressibleMixturePropertyPackage
...
#Region "    DWSIM Functions"
        ...
        ''' </summary>
        Private Function ExtrapolateAlongTPure(fluid As String, prop As String, T As Double, P As Double,
                                               Tmin As Double, Tmax As Double, vaporSide As Boolean,
                                               Optional TsatHint As Double = Double.NaN) As Double
            Dim eT As Double = Math.Max(0.5, 0.001 * (Tmax - Tmin))
            Dim Tsat As Double = TsatHint
            If Double.IsNaN(Tsat) Then
                Try
                    Dim Pc As Double = CoolProp.Props1SI(fluid, "PCRIT")
                    If P < Pc Then
                        Tsat = CoolProp.PropsSI("T", "P", P, "Q", If(vaporSide, 1.0, 0.0), fluid)
                    End If
                Catch
                    Tsat = Double.NaN
                End Try
            ...
            If vaporSide Then
                Dim lower As Double = If(Double.IsNaN(Tsat), Tmin + eT, Math.Min(Tsat + eT, Tmax - eT))
                Dim upper As Double = Tmax - eT
                If lower >= upper Then lower = Math.Max(Tmin + eT, upper - eT)
                Ta = lower
                Tb = Math.Min(upper, lower + eT)
                If Tb <= Ta Then Tb = lower + 0.000001
            Else
                Dim upper As Double = If(Double.IsNaN(Tsat), Tmax - eT, Math.Max(Math.Min(Tsat - eT, Tmax - eT), Tmin + eT))
                Dim lower As Double = Tmin + eT
                If upper <= lower Then upper = Math.Min(Tmax - eT, lower + eT)
                Ta = upper
                Tb = Math.Max(lower, upper - eT)
                If Tb >= Ta Then Tb = upper - 0.000001
            End If

            Dim fA As Double, fB As Double
            Try
                fA = CoolProp.PropsSI(prop, "T", Ta, "P", P, fluid)
            Catch
                Return Double.NaN
            End Try
            Try
                fB = CoolProp.PropsSI(prop, "T", Tb, "P", P, fluid)
            Catch
                fB = fA
                Tb = Ta
            End Try
            ...
            If Math.Abs(Tb - Ta) > 0.00000001 Then slope = (fB - fA) / (Tb - Ta)
            Return fA + slope * (T - Ta)
        End Function
            ...
            If st = State.Liquid Then
                If x < xmax Then
                    ...
                    Catch ex As Exception
                        Dim Tsat As Double = Double.NaN
                        Try : Tsat = AUX_TSAT(P, x) : Catch : End Try
                        Return ExtrapolateAlongTPure(GetCoolPropName(x), "H", T, P, Tmin, Tmax, False, Tsat) / 1000
                    End Try
                Else
                    Try
                        Return CoolProp.PropsSI("H", "T", T, "P", P, GetCoolPropName(xmax)) / 1000
                    Catch ex As Exception
                        Dim Tsat As Double = Double.NaN
                        Try : Tsat = AUX_TSAT(P, xmax) : Catch : End Try
                        Return ExtrapolateAlongTPure(GetCoolPropName(xmax), "H", T, P, Tmin, Tmax, False, Tsat) / 1000
                    End Try
                End If
            Else
