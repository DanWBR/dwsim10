...
Namespace PropertyPackages
    ...
    <System.Serializable()> Public Partial Class PRSV2VLPropertyPackage
...
#Region "   Analytical Derivatives"
        ...
        ' Only the temperature derivative is analytical, as for PRSV2: this model
        ' adds a composition-dependent mixing rule on top, which makes d(lnphi)/dn
        ' harder still and leaves it on finite differences.
        '
        ' The model routine hands back Nothing when its pressure-correction branch
        ' is in play, because what that branch computes is not the expression the
        ' derivative was taken of. Falling back then is not a nicety - it is the
        ' difference between an approximate derivative and a wrong one.
        ...
        Public Overrides Function DW_CalcdLnFugCoeffdT(ByVal Vx As Double(), ByVal T As Double, ByVal P As Double, st As State) As Double()
            ...
            Dim d = m_pr.CalcLnFugDT(T, P, Vx, RET_VKij, RET_VKij2, RET_KAPPA1, RET_KAPPA2, RET_KAPPA3, RET_VTC, RET_VPC, RET_VW, TIPO)
        ...
        Public Overrides Function DW_CalcdKdT(ByVal Vx As Double(), ByVal Vy As Double(), ByVal T As Double, ByVal P As Double, Optional ByVal type As String = "LV") As Double()
            ...
            Dim dLdT = m_pr.CalcLnFugDT(T, P, Vx, RET_VKij, RET_VKij2, RET_KAPPA1, RET_KAPPA2, RET_KAPPA3, RET_VTC, RET_VPC, RET_VW, "L")
            Dim dVdT = m_pr.CalcLnFugDT(T, P, Vy, RET_VKij, RET_VKij2, RET_KAPPA1, RET_KAPPA2, RET_KAPPA3, RET_VTC, RET_VPC, RET_VW, "V")
            If dLdT Is Nothing OrElse dVdT Is Nothing Then Return MyBase.DW_CalcdKdT(Vx, Vy, T, P, type)
