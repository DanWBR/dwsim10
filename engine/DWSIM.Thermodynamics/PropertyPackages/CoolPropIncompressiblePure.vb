...
Namespace PropertyPackages
    ...
    <System.Serializable()> Public Partial Class CoolPropIncompressiblePurePropertyPackage
...
#Region "    DWSIM Functions"
        ...
        Public Overrides Function AUX_CONDTL(T As Double, Optional phaseid As Integer = 3) As Double
            ...
            Catch ex As Exception
                Return ExtrapolateAlongT("L", T, 101325.0, Tmin, Tmax)
            End Try
        ...
        Public Overrides Function AUX_LIQDENS(T As Double, Vx As System.Array, Optional P As Double = 0.0, Optional Pvp As Double = 0.0, Optional FORCE_EOS As Boolean = False) As Double
            ...
            Catch ex As Exception
                Return ExtrapolateAlongT("D", T, P, Tmin, Tmax)
            End Try
        ...
        Public Overrides Function AUX_SURFTM(T As Double) As Double
            ...
            Catch ex As Exception
                Return ExtrapolateAlongT("I", T, 101325.0, Tmin, Tmax)
            End Try
        ...
        Public Function AUX_VAPVISCMIX(T As Double, P As Double, MM As Double) As Double
            ...
            Catch ex As Exception
                Return ExtrapolateAlongT("V", T, 101325.0, Tmin, Tmax)
            End Try
        ...
        ''' <summary>
        ''' Linear T-extrapolation along an isobar for an incompressible CoolProp fluid. Anchors are kept
        ''' strictly inside [Tmin..Tmax] and below the saturation temperature at the same pressure, so
        ''' CoolProp is never queried out of range.
        ''' </summary>
        Private Function ExtrapolateAlongT(prop As String, T As Double, P As Double, Tmin As Double, Tmax As Double) As Double
            ...
            Catch
                Tsat = Tmax
            End Try
            Dim Tcap As Double = Math.Min(Tmax - eT, Tsat - eT)
            If Tcap <= Tmin + eT Then Tcap = Tmax - eT

            Dim Ta As Double, Tb As Double
            If T > Tcap Then
                Ta = Tcap
                Tb = Math.Max(Tmin + eT, Tcap - eT)
            ElseIf T < Tmin + eT Then
                Ta = Tmin + eT
                Tb = Math.Min(Tcap, Tmin + 2.0 * eT)
            Else
                Ta = T
                Tb = Math.Min(Tcap, T + eT)
                If Tb <= Ta Then Tb = Math.Max(Tmin + eT, T - eT)
            End If

            Dim fA As Double, fB As Double
            Try
                fA = CoolProp.PropsSI(prop, "T", Ta, "P", P, GetCoolPropName())
            Catch ex As Exception
                Return Double.NaN
            End Try
            Try
                fB = CoolProp.PropsSI(prop, "T", Tb, "P", P, GetCoolPropName())
            Catch ex As Exception
                fB = fA
                Tb = Ta
            End Try
            ...
            If Math.Abs(Tb - Ta) > 1.0E-08 Then slope = (fB - fA) / (Tb - Ta)
        ...
        Public Overrides Function DW_CalcCp_ISOL(ByVal Phase1 As Phase, ByVal T As Double, ByVal P As Double) As Double
            ...
            Catch ex As Exception
                Return ExtrapolateAlongT("C", T, P, Tmin, Tmax) / 1000
            End Try
        ...
        Public Overrides Function DW_CalcEnthalpy(ByVal Vx As System.Array, ByVal T As Double, ByVal P As Double, ByVal st As State) As Double
            ...
            If st = State.Liquid Then
                ...
                Catch ex As Exception
                    Return ExtrapolateAlongT("H", T, P, Tmin, Tmax) / 1000
                End Try
            Else
                Return 0.0
            End If
        ...
        Public Overrides Function DW_CalcEntropy(ByVal Vx As System.Array, ByVal T As Double, ByVal P As Double, ByVal st As State) As Double
            ...
            If st = State.Liquid Then
                ...
                Catch ex As Exception
                    Return ExtrapolateAlongT("S", T, P, Tmin, Tmax) / 1000
                End Try
            Else
                Return 0.0
            End If
