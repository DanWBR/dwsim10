...
Namespace PropertyPackages
    ...
    <System.Runtime.InteropServices.Guid(IdealElectrolytePropertyPackage.ClassId)>
    <System.Serializable()> Public Partial Class IdealElectrolytePropertyPackage

        Inherits PropertyPackages.ElectrolyteBasePropertyPackage

        Public Shadows Const ClassId As String = "35A1D528-F696-440D-953E-977208096280"
        ...
        Public Sub New(ByVal comode As Boolean)
            ...
            ComponentName = "Ideal Solution (Aqueous Electrolytes)"
            ComponentDescription = "Ideal Aqueous Electrolyte Solution model"
...
#Region "    DWSIM Functions"
        ...
        Public Overrides Sub DW_CalcProp(ByVal [property] As String, ByVal phase As Phase)
            ...
            Select Case [property].ToLower
                ...
                Case "enthalpy", "enthalpynf"
                    ...
                    ElseIf phase = Phase.Vapor Then
                        result = Me.m_elec.LiquidEnthalpy(T, RET_VMOL(phase), constprops, RET_UnitaryVector(), RET_UnitaryVector(), False)
                ...
                Case "osmoticcoefficient"
                    ...
                    For Each su As Interfaces.ICompound In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                        constprops.Add(su.ConstantProperties)
                    Next
                    Me.CurrentMaterialStream.Phases(phaseID).Properties.osmoticCoefficient = Me.m_elec.OsmoticCoeff(RET_VMOL(phase), RET_UnitaryVector(), constprops)
                Case "freezingpoint"
                    Dim constprops As New List(Of Interfaces.ICompoundConstantProperties)
                    For Each su As Interfaces.ICompound In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                        constprops.Add(su.ConstantProperties)
                    Next
                    Me.CurrentMaterialStream.Phases(phaseID).Properties.freezingPoint = Me.m_elec.FreezingPointDepression(RET_VMOL(phase), RET_UnitaryVector(), constprops)(0)
                Case "freezingpointdepression"
                    Dim constprops As New List(Of Interfaces.ICompoundConstantProperties)
                    For Each su As Interfaces.ICompound In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                        constprops.Add(su.ConstantProperties)
                    Next
                    Me.CurrentMaterialStream.Phases(phaseID).Properties.freezingPointDepression = Me.m_elec.FreezingPointDepression(RET_VMOL(phase), RET_UnitaryVector(), constprops)(1)
                Case "ph"
                    Dim constprops As New List(Of Interfaces.ICompoundConstantProperties)
                    For Each su As Interfaces.ICompound In Me.CurrentMaterialStream.Phases(0).Compounds.Values
                        constprops.Add(su.ConstantProperties)
                    Next
                    Me.CurrentMaterialStream.Phases(phaseID).Properties.pH = Me.m_elec.pH(RET_VMOL(phase), T, RET_UnitaryVector(), constprops)
                Case Else
        ...
        Public Overrides Function DW_CalcEnthalpy(ByVal Vx As System.Array, ByVal T As Double, ByVal P As Double, ByVal st As State) As Double
            ...
            Select Case st
                Case State.Liquid
                    H = Me.RET_Hid(298.15, T, Vx) - Me.RET_HVAPM(Me.AUX_CONVERT_MOL_TO_MASS(Vx), T) - Me.m_elec.LiquidEnthalpy(T, Vx, constprops, RET_UnitaryVector(), RET_UnitaryVector(), False)
                Case State.Solid
                    H = Me.RET_Hid(298.15, T, Vx) - Me.RET_HVAPM(Me.AUX_CONVERT_MOL_TO_MASS(Vx), T) - Me.m_elec.LiquidEnthalpy(T, Vx, constprops, RET_UnitaryVector(), RET_UnitaryVector(), False) - Me.RET_HFUSM(Me.AUX_CONVERT_MOL_TO_MASS(Vx), T) - Me.m_elec.SolidEnthalpy(T, Vx, constprops)
                Case State.Vapor
