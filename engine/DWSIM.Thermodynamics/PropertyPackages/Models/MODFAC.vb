...
Namespace PropertyPackages.Auxiliary
    ...
    <System.Serializable()> Public Class Modfac
        ...
        Function GAMMA_MR(ByVal T As Double, ByVal Vx As Double(), ByVal VQ As Double(), ByVal VR As Double(), ByVal VEKI As List(Of Dictionary(Of Integer, Double))) As Double()
            ...
            Inspector.Host.CheckAndAdd(IObj, "", "GAMMA_MR", "Modified UNIFAC (Dortmund) Activity Coefficient", "MODFAC-Do Activity Coefficient Calculation Routine")
        ...
        ''' <summary>
        ''' Closed-form Modified UNIFAC (Dortmund) activity-coefficient derivatives. Returns
        ''' Object(){ gamma() , dln(gamma)/dT() , dln(gamma)/dn(,) } (mole-number, total-moles = 1 basis).
        ''' Same solution-of-groups form as GAMMA_MR: modified combinatorial (volume term uses r^(3/4)) and
        ''' temperature-dependent tau (d(ln tau)/dT from TAU_DT). Validated against finite differences.
        ''' </summary>
        ...
        ''' d(ln tau)/dT = a/T^2 - c = (a - c*T^2)/T^2. Mirrors TAU's interaction-parameter lookup.
        ''' </summary>
        Function TAU_DT(ByVal group_1 As Integer, ByVal group_2 As Integer, ByVal T As Double) As Double
            ...
            If g1 <> g2 Then
                If Me.ModfGroups.InteracParam_aij.ContainsKey(g1) Then
                    ...
                    ElseIf Me.ModfGroups.InteracParam_aij.ContainsKey(g2) Then
                        If Me.ModfGroups.InteracParam_aij(g2).ContainsKey(g1) Then
                            aa = Me.ModfGroups.InteracParam_aji(g2)(g1) : cc = Me.ModfGroups.InteracParam_cji(g2)(g1)
                        End If
                    End If
                ElseIf Me.ModfGroups.InteracParam_aij.ContainsKey(g2) Then
                    If Me.ModfGroups.InteracParam_aij(g2).ContainsKey(g1) Then
