...
Namespace PropertyPackages.Auxiliary
    ...
    <System.Serializable()> Public Class NISTMFAC
        ...
        ''' <summary>
        ''' d(ln tau)/dT for a group pair, where tau = exp(-(a + b*T + c*T^2)/T). Since ln tau = -a/T - b - c*T,
        ''' d(ln tau)/dT = a/T^2 - c = (a - c*T^2)/T^2. Mirrors TAU's interaction-parameter lookup (the NIST
        ''' variant stores both directions in the same aij/bij/cij dictionaries).
        ''' </summary>
        Function TAU_DT(ByVal group_1 As Integer, ByVal group_2 As Integer, ByVal T As Double) As Double

            Dim g1 As Integer = Me.ModfGroups.Groups(group_1).PrimaryGroup
            Dim g2 As Integer = Me.ModfGroups.Groups(group_2).PrimaryGroup
            Dim aa As Double = 0.0, cc As Double = 0.0

            If g1 <> g2 Then
                If Me.ModfGroups.InteracParam_aij.ContainsKey(g1) Then
                    If Me.ModfGroups.InteracParam_aij(g1).ContainsKey(g2) Then
                        aa = Me.ModfGroups.InteracParam_aij(g1)(g2) : cc = Me.ModfGroups.InteracParam_cij(g1)(g2)
                    ElseIf Me.ModfGroups.InteracParam_aij.ContainsKey(g2) Then
                        If Me.ModfGroups.InteracParam_aij(g2).ContainsKey(g1) Then
                            aa = Me.ModfGroups.InteracParam_aij(g2)(g1) : cc = Me.ModfGroups.InteracParam_cij(g2)(g1)
                        End If
                    End If
                ElseIf Me.ModfGroups.InteracParam_aij.ContainsKey(g2) Then
                    If Me.ModfGroups.InteracParam_aij(g2).ContainsKey(g1) Then
            ...
            Return (aa - cc * T ^ 2) / T ^ 2

        End Function

        ''' <summary>
        ''' Closed-form derivatives of the group-contribution activity coefficients for Modified UNIFAC (NIST):
        ''' returns { gamma, d(ln gamma)/dT, d(ln gamma)/dn (mole-number basis) }. Same solution-of-groups
        ''' assembly as GAMMA_MR; the temperature dependence enters only through tau (via TAU_DT above), and the
        ''' composition derivative through the group area fractions theta and the group sums s_k. The
        ''' combinatorial part uses the Dortmund/NIST r^(3/4) volume term. Mirrors the MODFAC implementation.
        ''' </summary>
        Function GAMMA_DERIVS(ByVal T As Double, ByVal Vx As Double(), ByVal VQ As Double(), ByVal VR As Double(), ByVal VEKI As List(Of Dictionary(Of Integer, Double))) As Object

            Dim n As Integer = Vx.Length - 1
            Dim i, p, gm, gk, gl As Integer

            Dim idset As New List(Of Integer)
            For Each comp In VEKI
                For Each kv In comp
                    If Not idset.Contains(kv.Key) Then idset.Add(kv.Key)
                Next
            Next
            Dim ids As Integer() = idset.ToArray
            Dim ng As Integer = ids.Length - 1

            Dim nu(n, ng) As Double
            For i = 0 To n
                For gk = 0 To ng
                    nu(i, gk) = If(VEKI(i).ContainsKey(ids(gk)), VEKI(i)(ids(gk)), 0.0)
                Next
            Next

            Dim tauG(ng, ng), dtauG(ng, ng) As Double
            For gk = 0 To ng
                For gm = 0 To ng
                    tauG(gk, gm) = TAU(ids(gk), ids(gm), T)
                    dtauG(gk, gm) = tauG(gk, gm) * TAU_DT(ids(gk), ids(gm), T)
                Next
            Next

            Dim beta(n, ng), dbeta(n, ng) As Double
            For i = 0 To n
                For gm = 0 To ng
                    For gk = 0 To ng
                        beta(i, gm) += nu(i, gk) * tauG(gk, gm)
                        dbeta(i, gm) += nu(i, gk) * dtauG(gk, gm)
                    Next
                Next
            Next

            Dim Q(n), Rr(n) As Double
            Dim soma_xq As Double = 0.0, soma_xr As Double = 0.0, soma_xr_ As Double = 0.0
            For i = 0 To n
                Q(i) = VQ(i) : Rr(i) = VR(i)
                soma_xq += Vx(i) * Q(i)
                soma_xr += Vx(i) * Rr(i)
                soma_xr_ += Vx(i) * Rr(i) ^ (3.0 / 4.0)
            Next

            Dim theta(ng), sgrp(ng), dsdT(ng) As Double
            For gm = 0 To ng
                For i = 0 To n
                    theta(gm) += Vx(i) * Q(i) * nu(i, gm) / soma_xq
                Next
            Next
            For gm = 0 To ng
                For gl = 0 To ng
                    sgrp(gm) += theta(gl) * tauG(gl, gm)
                    dsdT(gm) += theta(gl) * dtauG(gl, gm)
                Next
            Next

            Dim JJ(n), JJ_(n), LL(n) As Double
            For i = 0 To n
                JJ(i) = Rr(i) / soma_xr
                JJ_(i) = Rr(i) ^ (3.0 / 4.0) / soma_xr_
                LL(i) = Q(i) / soma_xq
            Next
            ...
            For i = 0 To n
                ...
                For gm = 0 To ng
                    Dim b As Double = beta(i, gm), db As Double = dbeta(i, gm), sg As Double = sgrp(gm), dsg As Double = dsdT(gm)
                    acc += theta(gm) * (db * sg - b * dsg) / (sg * sg)
                    If nu(i, gm) <> 0.0 Then acc -= nu(i, gm) * (db / b - dsg / sg)
                Next
                dlngdT(i) = -Q(i) * acc
            Next
            ...
            For p = 0 To n
                Dim dtheta(ng), dsdx(ng) As Double
                For gm = 0 To ng
                    dtheta(gm) = Q(p) * (nu(p, gm) - theta(gm)) / soma_xq
                Next
                For gm = 0 To ng
                    For gl = 0 To ng
                        dsdx(gm) += dtheta(gl) * tauG(gl, gm)
                    Next
                Next
                For i = 0 To n
                    ' modified combinatorial: volume term uses r^(3/4) (JJ_), the q-term uses r (JJ)
                    Dim dJ As Double = -Rr(i) * Rr(p) / (soma_xr * soma_xr)
                    Dim dJ_ As Double = -(Rr(i) ^ (3.0 / 4.0)) * (Rr(p) ^ (3.0 / 4.0)) / (soma_xr_ * soma_xr_)
                    Dim dL As Double = -Q(i) * Q(p) / (soma_xq * soma_xq)
                    Dim JL As Double = JJ(i) / LL(i)
                    Dim dJL As Double = (dJ * LL(i) - JJ(i) * dL) / (LL(i) * LL(i))
                    Dim dgc As Double = -dJ_ + dJ_ / JJ_(i) - 5 * Q(i) * (-dJL + dJL / JL)
                    Dim acc As Double = 0.0
                    For gm = 0 To ng
                        Dim b As Double = beta(i, gm), sg As Double = sgrp(gm)
                        acc += dtheta(gm) * b / sg - theta(gm) * b * dsdx(gm) / (sg * sg)
                        If nu(i, gm) <> 0.0 Then acc += nu(i, gm) * dsdx(gm) / sg
                    Next
                    Dim dgr As Double = -Q(i) * acc
                    dxu(i, p) = dgc + dgr
                Next
            Next

            Dim dlngdx(n, n) As Double
            For i = 0 To n
                Dim proj As Double = 0.0
                For p = 0 To n : proj += Vx(p) * dxu(i, p) : Next
                For p = 0 To n : dlngdx(i, p) = dxu(i, p) - proj : Next
            Next

            Dim gamma = DirectCast(GAMMA_MR(T, Vx, VQ, VR, VEKI), Double())

            Return New Object() {gamma, dlngdT, dlngdx}
