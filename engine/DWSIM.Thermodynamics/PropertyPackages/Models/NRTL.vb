...
Namespace PropertyPackages.Auxiliary
    ...
    <System.Serializable()> Public Class NRTL
        ...
        ''' <summary>
        ''' Closed-form activity-coefficient derivatives for the NRTL model. Returns
        ''' Object(){ gamma() , dln(gamma)/dT() , dln(gamma)/dx(,) } computed from the same tau/G/S/C
        ''' intermediates as GAMMA_MR. dtau_ij/dT = (C12*T^2 - A12)/(1.98721*T^2). Validated against
        ''' finite differences of GAMMA_MR in DWSIM.Thermodynamics.Derivatives.Tests.
        ''' </summary>
        Function GAMMA_DERIVS(ByVal T As Double, ByVal Vx As Double(), ByVal Vids As String()) As Object

            Dim n As Integer = Vx.Length - 1
            Dim tau_ij(n)(), tau_ji(n)(), alpha(n)(), dtij(n)(), dtji(n)() As Double
            Dim i, j, k As Integer
            For i = 0 To n
                Array.Resize(tau_ij(i), n + 1)
                Array.Resize(tau_ji(i), n + 1)
                Array.Resize(alpha(i), n + 1)
                Array.Resize(dtij(i), n + 1)
                Array.Resize(dtji(i), n + 1)
            Next
            ...
            For i = 0 To n
                For j = 0 To n
                    Dim aij, bij, cij, aji, bji, cji, al As Double
                    ...
                    If Me.InteractionParameters.ContainsKey(Vids(i)) AndAlso Me.InteractionParameters(Vids(i)).ContainsKey(Vids(j)) Then
                        ...
                        aij = ip.A12 : bij = ip.B12 : cij = ip.C12
                        aji = ip.A21 : bji = ip.B21 : cji = ip.C21
                        al = ip.alpha12 : found = True
                    ElseIf Me.InteractionParameters.ContainsKey(Vids(j)) AndAlso Me.InteractionParameters(Vids(j)).ContainsKey(Vids(i)) Then
                        ...
                        aij = ip.A21 : bij = ip.B21 : cij = ip.C21
                        aji = ip.A12 : bji = ip.B12 : cji = ip.C12
                        al = ip.alpha12 : found = True
                    End If
                    If found Then
                        tau_ij(i)(j) = (aij + bij * T + cij * T * T) / (RC * T)
                        tau_ji(i)(j) = (aji + bji * T + cji * T * T) / (RC * T)
                        dtij(i)(j) = (cij * T * T - aij) / (RC * T * T)
                        dtji(i)(j) = (cji * T * T - aji) / (RC * T * T)
                        alpha(i)(j) = al
                    End If
                Next
            Next

            Dim Gij(n)(), Gji(n)(), dGij(n)(), dGji(n)() As Double
            For i = 0 To n
                Array.Resize(Gij(i), n + 1)
                Array.Resize(Gji(i), n + 1)
                Array.Resize(dGij(i), n + 1)
                Array.Resize(dGji(i), n + 1)
                For j = 0 To n
                    Gij(i)(j) = Math.Exp(-alpha(i)(j) * tau_ij(i)(j))
                    Gji(i)(j) = Math.Exp(-alpha(i)(j) * tau_ji(i)(j))
                    dGij(i)(j) = -alpha(i)(j) * Gij(i)(j) * dtij(i)(j)
                    dGji(i)(j) = -alpha(i)(j) * Gji(i)(j) * dtji(i)(j)
                Next
            Next

            Dim S(n), C(n), dSdT(n), dCdT(n) As Double
            For i = 0 To n
                For k = 0 To n
                    S(i) += Vx(k) * Gji(i)(k)
                    C(i) += Vx(k) * Gji(i)(k) * tau_ji(i)(k)
                    dSdT(i) += Vx(k) * dGji(i)(k)
                    dCdT(i) += Vx(k) * (dGji(i)(k) * tau_ji(i)(k) + Gji(i)(k) * dtji(i)(k))
                Next
            Next
            ...
            For i = 0 To n
                lng(i) = C(i) / S(i)
                dlngdT(i) = (dCdT(i) * S(i) - C(i) * dSdT(i)) / (S(i) * S(i))
                For j = 0 To n
                    Dim u As Double = Gij(i)(j), du As Double = dGij(i)(j)
                    Dim v As Double = tau_ij(i)(j) - C(j) / S(j)
                    Dim dv As Double = dtij(i)(j) - (dCdT(j) * S(j) - C(j) * dSdT(j)) / (S(j) * S(j))
                    Dim g As Double = 1.0 / S(j), dg As Double = -dSdT(j) / (S(j) * S(j))
                    lng(i) += Vx(j) * u * v * g
                    dlngdT(i) += Vx(j) * (du * v * g + u * dv * g + u * v * dg)
                Next
            Next

            ' Unconstrained d(ln gamma_i)/dx_m (treating each x_m as independent)
            Dim dxu(n, n) As Double
            For i = 0 To n
                For k = 0 To n ' k = m (perturbed component)
                    Dim dSi As Double = Gji(i)(k), dCi As Double = Gji(i)(k) * tau_ji(i)(k)
                    Dim val As Double = (dCi * S(i) - C(i) * dSi) / (S(i) * S(i))
                    For j = 0 To n
                        Dim u As Double = Gij(i)(j)
                        Dim v As Double = tau_ij(i)(j) - C(j) / S(j)
                        Dim g As Double = 1.0 / S(j)
                        Dim inner As Double = u * v * g
                        Dim dCj As Double = Gji(j)(k) * tau_ji(j)(k), dSj As Double = Gji(j)(k)
                        Dim dv As Double = -(dCj * S(j) - C(j) * dSj) / (S(j) * S(j))
                        Dim dg As Double = -dSj / (S(j) * S(j))
                        Dim dinner As Double = u * (dv * g + v * dg)
                        Dim kron As Double = If(j = k, 1.0, 0.0)
                        val += kron * inner + Vx(j) * dinner
                    Next
                    dxu(i, k) = val
                Next
            Next

            ' Convert to mole-number (total moles = 1) basis: d(ln gamma_i)/dn_m = dxu(i,m) - sum_k x_k dxu(i,k)
