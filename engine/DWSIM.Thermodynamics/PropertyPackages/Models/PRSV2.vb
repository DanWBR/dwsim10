...
Final3:
        ...
        ''' <summary>
        ''' Analytical d(ln fugacity coefficient)/dT for PRSV2. Mirrors CalcLnFug (temperature-dependent
        ''' kappa(Tr), composition-dependent Stryjek-Vera mixing rule a(i,j) with two kij matrices, and the
        ''' partial-molar aml2 = sum1+sum2+sum3), differentiating the PR-form fugacity through T. The Pcorr
        ''' root-existence correction (Log(Pcorr/P)) is zero wherever the requested phase root exists, i.e.
        ''' throughout the normal two-phase region, and is not differentiated. Validated against finite
        ''' differences of CalcLnFug.
        ''' </summary>
        Function CalcLnFugDT(ByVal T As Double, ByVal P As Double, ByVal Vx() As Double,
                             ByVal VKij(,) As Double, ByVal VKij2(,) As Double,
                             ByVal Vk1 As Double(), ByVal Vk2 As Double(), ByVal Vk3 As Double(),
                             ByVal VTc As Double(), ByVal VPc As Double(), ByVal Vw As Double(), ByVal TIPO As String) As Double()
            ...
            For i = 0 To n
                ...
                Else
                    ...
                    dcidT = 0.0
                End If
                Dim s As Double = 1 + ci * (1 - sqTr)
                ai(i) = 0.45724 * (s * s) * R ^ 2 * Tc ^ 2 / VPc(i)
                bi(i) = 0.0778 * R * Tc / VPc(i)
                Dim dsdT As Double = dcidT * (1 - sqTr) - ci / (2 * Math.Sqrt(T * Tc))
                daidT(i) = 2 * ai(i) * dsdT / s
            Next

            ' composition-dependent mixing and its T-derivative
            ...
            Dim sum1(n), sum2(n), sum3(n), dsum1(n), dsum2(n), dsum3(n) As Double
            For i = 0 To n
                For j = 0 To n
                    If i <> j Then
                        Dim sq As Double = (a(i, i) * a(j, j)) ^ 0.5
                        Dim dsq As Double = sq * 0.5 * (daidT(i) / ai(i) + daidT(j) / ai(j))
                        Dim f2 As Double = Vx(i) * Vx(j) * (Vx(j) * VKij2(j, i) - (1 - Vx(i)) * VKij(i, j))
                        sum2(i) += f2 * sq
                        dsum2(i) += f2 * dsq
                    End If
                    For k = 0 To n
                        If i <> j And k > j And k <> i Then
                            ...
                            Dim f3 As Double = Vx(j) * Vx(k) * (Vx(j) * VKij(j, k) + Vx(k) * VKij2(k, j))
                            sum3(i) += f3 * sq
                            dsum3(i) += f3 * dsq
                        End If
                    Next
