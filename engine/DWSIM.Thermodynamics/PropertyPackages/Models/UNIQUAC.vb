...
Namespace PropertyPackages.Auxiliary
    ...
    <System.Serializable()> Public Class UNIQUAC
        ...
        ''' <summary>
        ''' Closed-form activity-coefficient derivatives for the UNIQUAC model. Returns
        ''' Object(){ gamma() , dln(gamma)/dT() , dln(gamma)/dn(,) } (composition on total-moles = 1 basis),
        ''' computed from the same combinatorial/residual intermediates as GAMMA_MR. The combinatorial part
        ''' is temperature-independent; tau_ij = exp((-a12+b12*T+c12*T^2)/(1.98721*T)) and
        ''' dtau_ij/dT = tau_ij*(a12+c12*T^2)/(1.98721*T^2). Validated against finite differences in
        ''' DWSIM.Thermodynamics.Derivatives.Tests.
        ''' </summary>
        Function GAMMA_DERIVS(ByVal T As Double, ByVal Vx As Double(), ByVal Vids As String(), ByVal VQ As Double(), ByVal VR As Double()) As Object

            Dim n As Integer = Vx.Length - 1
            Dim RC As Double = 1.98721
            Dim z As Double = 10.0

            Dim a12(n)(), a21(n)(), b12(n)(), b21(n)(), c12(n)(), c21(n)() As Double
            Dim tau_ij(n)(), tau_ji(n)(), dtij(n)(), dtji(n)() As Double
            Dim i, j, l, mm As Integer
            For i = 0 To n
                Array.Resize(a12(i), n + 1) : Array.Resize(a21(i), n + 1)
                Array.Resize(b12(i), n + 1) : Array.Resize(b21(i), n + 1)
                Array.Resize(c12(i), n + 1) : Array.Resize(c21(i), n + 1)
                Array.Resize(tau_ij(i), n + 1) : Array.Resize(tau_ji(i), n + 1)
                Array.Resize(dtij(i), n + 1) : Array.Resize(dtji(i), n + 1)
            Next

            For i = 0 To n
                For j = 0 To n
                    If Me.InteractionParameters.ContainsKey(Vids(i)) AndAlso Me.InteractionParameters(Vids(i)).ContainsKey(Vids(j)) Then
                        Dim ip = Me.InteractionParameters(Vids(i))(Vids(j))
                        a12(i)(j) = ip.A12 : a21(i)(j) = ip.A21
                        b12(i)(j) = ip.B12 : b21(i)(j) = ip.B21
                        c12(i)(j) = ip.C12 : c21(i)(j) = ip.C21
                    ElseIf Me.InteractionParameters.ContainsKey(Vids(j)) AndAlso Me.InteractionParameters(Vids(j)).ContainsKey(Vids(i)) Then
                        Dim ip = Me.InteractionParameters(Vids(j))(Vids(i))
                        a12(i)(j) = ip.A21 : a21(i)(j) = ip.A12
                        b12(i)(j) = ip.B21 : b21(i)(j) = ip.B12
                        c12(i)(j) = ip.C21 : c21(i)(j) = ip.C12
                    End If
                    tau_ij(i)(j) = Math.Exp((-a12(i)(j) + b12(i)(j) * T + c12(i)(j) * T * T) / (RC * T))
                    tau_ji(i)(j) = Math.Exp((-a21(i)(j) + b21(i)(j) * T + c21(i)(j) * T * T) / (RC * T))
                    dtij(i)(j) = tau_ij(i)(j) * (a12(i)(j) + c12(i)(j) * T * T) / (RC * T * T)
                    dtji(i)(j) = tau_ji(i)(j) * (a21(i)(j) + c21(i)(j) * T * T) / (RC * T * T)
                Next
            Next

            Dim r As Double = 0.0, q As Double = 0.0
            For i = 0 To n
                r += Vx(i) * VR(i)
                q += Vx(i) * VQ(i)
            Next
            Dim teta(n), fi(n) As Double
            For i = 0 To n
                fi(i) = Vx(i) * VR(i) / r
                teta(i) = Vx(i) * VQ(i) / q
            Next

            ' S(i) = sum_l teta_l tau_ji(i)(l); dS/dT
            Dim S(n), dSdT(n) As Double
            For i = 0 To n
                For l = 0 To n
                    S(i) += teta(l) * tau_ji(i)(l)
                    dSdT(i) += teta(l) * dtji(i)(l)
                Next
            Next
            ' sum1(i) = sum_l teta_l tau_ij(i)(l)/S(l); dsum1/dT
            Dim sum1(n), dsum1dT(n) As Double
            For i = 0 To n
                For l = 0 To n
                    sum1(i) += teta(l) * tau_ij(i)(l) / S(l)
                    dsum1dT(i) += teta(l) * (dtij(i)(l) / S(l) - tau_ij(i)(l) * dSdT(l) / (S(l) * S(l)))
                Next
            Next

            Dim lng(n), dlngdT(n) As Double
            For i = 0 To n
                Dim rho As Double = fi(i) / teta(i)
                Dim lngc As Double = 1.0 - VR(i) / r + Math.Log(VR(i) / r) - (z / 2.0) * VQ(i) * (1.0 - rho + Math.Log(rho))
                Dim lngr As Double = VQ(i) * (1.0 - Math.Log(S(i)) - sum1(i))
                lng(i) = lngc + lngr
                ' combinatorial is T-independent
                dlngdT(i) = VQ(i) * (-dSdT(i) / S(i) - dsum1dT(i))
            Next

            ' Unconstrained d(ln gamma_i)/dx_m, then convert to mole-number (N=1) basis
            Dim dxu(n, n) As Double
            For mm = 0 To n
                Dim drdx As Double = VR(mm), dqdx As Double = VQ(mm)
                ' dtheta_l/dx_m and dS_l/dx_m
                Dim dtheta(n), dSx(n) As Double
                For l = 0 To n
                    Dim kron As Double = If(l = mm, 1.0, 0.0)
                    dtheta(l) = kron * VQ(l) / q - teta(l) * dqdx / q
                Next
                For i = 0 To n
                    dSx(i) = 0.0
                    For l = 0 To n
                        dSx(i) += dtheta(l) * tau_ji(i)(l)
                    Next
                Next
                For i = 0 To n
                    ' combinatorial: rho_i = (R_i/Q_i)(q/r)
                    Dim rho As Double = fi(i) / teta(i)
                    Dim drho As Double = (VR(i) / VQ(i)) * (dqdx * r - q * drdx) / (r * r)
                    Dim dlngc As Double = VR(i) * drdx / (r * r) - drdx / r - (z / 2.0) * VQ(i) * drho * (1.0 / rho - 1.0)
                    ' residual
                    Dim dsum1x As Double = 0.0
                    For l = 0 To n
                        dsum1x += dtheta(l) * tau_ij(i)(l) / S(l) - teta(l) * tau_ij(i)(l) * dSx(l) / (S(l) * S(l))
                    Next
                    Dim dlngr As Double = VQ(i) * (-dSx(i) / S(i) - dsum1x)
                    dxu(i, mm) = dlngc + dlngr
                Next
            Next
            ...
            For i = 0 To n
                ...
                Next
                For mm = 0 To n
                    dlngdx(i, mm) = dxu(i, mm) - proj
                Next
            Next

            Dim gamma(n) As Double
            For i = 0 To n
                gamma(i) = Math.Exp(lng(i))
            Next
