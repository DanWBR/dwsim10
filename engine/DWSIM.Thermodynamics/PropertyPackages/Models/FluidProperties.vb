...
Namespace PropertyPackages.Auxiliary
    ...
    <System.Serializable()> Public Class PROPS
        ...
        Shared Function liq_dens_COSTALD_COMPR(x() As Double, T As Double, V_char() As Double, Tc() As Double, omega_SRK() As Double, P As Double) As Double
            ...
            ' Tait correction (Eq. 3-11.5) requires both (BETA+P) and (BETA+P_vap) > 0;
            ' near the critical point BETA can be negative enough to violate this, producing log(<=0) = NaN.
            ' Fall back to the saturated value when out of domain.
            If (BETA + P) <= 0.0 Or (BETA + P_VAP_mix) <= 0.0 Then
                Return V_sat
            End If
