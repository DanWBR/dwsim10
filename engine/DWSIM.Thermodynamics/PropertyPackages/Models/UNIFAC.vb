...
Namespace PropertyPackages.Auxiliary
    ...
    <System.Serializable()> Public Class Unifac
        ...
        ''' <summary>
        ''' Closed-form UNIFAC activity-coefficient derivatives. Returns Object(){ gamma() ,
        ''' dln(gamma)/dT() , dln(gamma)/dn(,) } (composition on total-moles = 1 basis), computed from the
        ''' same solution-of-groups intermediates as GAMMA_MR. Only tau_km = exp(-a_km/T) depends on T, and
        ''' d(tau)/dT = -tau*ln(tau)/T. Validated against finite differences of GAMMA_MR.
        ''' </summary>
        Function GAMMA_DERIVS(ByVal T As Double, ByVal Vx As Double(), ByVal VQ As Double(), ByVal VR As Double(), ByVal VEKI As List(Of Dictionary(Of Integer, Double))) As Object
            ...
            For gk = 0 To ng
                For gm = 0 To ng
                    ...
                    dtauG(gk, gm) = -tauG(gk, gm) * Math.Log(tauG(gk, gm)) / T
                Next
            Next
            ...
            Dim soma_xq As Double = 0.0, soma_xr As Double = 0.0
            For i = 0 To n
                Q(i) = VQ(i) : Rr(i) = VR(i)
                soma_xq += Vx(i) * Q(i) : soma_xr += Vx(i) * Rr(i)
            Next
            ...
            Dim JL_J(n), JL_L(n) As Double
            For i = 0 To n
                JL_J(i) = Rr(i) / soma_xr
                JL_L(i) = Q(i) / soma_xq
            Next

            ' d(ln gamma)/dT (combinatorial is T-independent)
            ...
            ' unconstrained d(ln gamma)/dx_p
            Dim dxu(n, n) As Double
            For p = 0 To n
                ' composition derivatives of the group fractions
                ...
                For i = 0 To n
                    ...
                    Dim JL As Double = JL_J(i) / JL_L(i)
                    Dim dJL As Double = (dJ * JL_L(i) - JL_J(i) * dL) / (JL_L(i) * JL_L(i))
                    Dim dgc As Double = -dJ + dJ / JL_J(i) - 5 * Q(i) * (-dJL + dJL / JL)
                    ' residual
                    Dim acc As Double = 0.0
                    For gm = 0 To ng
