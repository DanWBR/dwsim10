...
Final3:
        ...
        ''' </summary>
        ''' <returns>
        ''' The derivative vector, or Nothing when the analytical route does not
        ''' apply and the caller should fall back to finite differences.
        ''' </returns>
        ''' <remarks>
        ''' Same shape as the PRSV2 routine, and for the same reason: the
        ''' Stryjek-Vera alpha carries all of the temperature dependence, while
        ''' this model's mixing rule - the composition-dependent term below - is a
        ''' factor on sqrt(a_i a_j) that does not see T at all. So every a(i,j),
        ''' and every sum built from products of a(i,i), keeps its composition
        ''' factor and differentiates through the a_i alone.
        '''
        ''' Nothing is returned when CheckRoot has moved the pressure. That branch
        ''' adds log(Pcorr/P) to the fugacity coefficient, and Pcorr comes out of a
        ''' root-existence fix that is not differentiable in any useful sense: the
        ''' function actually being differentiated is then not the expression
        ''' assembled here.
        ''' </remarks>
        Function CalcLnFugDT(ByVal T As Double, ByVal P As Double, ByVal Vx() As Double,
                             ByVal VKij(,) As Double, ByVal VKij2(,) As Double,
                             ByVal Vk1 As Double(), ByVal Vk2 As Double(), ByVal Vk3 As Double(),
                             ByVal VTc As Double(), ByVal VPc As Double(), ByVal Vw As Double(),
                             ByVal TIPO As String) As Double()

            Dim n As Integer = Vx.Length - 1
            Const R As Double = 8.314
            Dim i, j, k As Integer

            Dim ai(n), bi(n), daidT(n) As Double
            For i = 0 To n
                Dim Tc As Double = VTc(i), Tr As Double = T / Tc, w As Double = Vw(i)
                Dim sqTr As Double = Math.Sqrt(Tr)
                Dim ci As Double, dcidT As Double
                If Vk1(i) * Vk2(i) * Vk3(i) <> 0.0 Then
                    Dim k0 As Double = 0.378893 + 1.4897153 * w - 0.17131848 * w ^ 2 + 0.0196544 * w ^ 3
                    Dim P1 As Double = Vk1(i) + Vk2(i) * (Vk3(i) - Tr) * (1 - sqTr)
                    Dim P2 As Double = (1 + sqTr) * (0.7 - Tr)
                    ci = k0 + P1 * P2
                    Dim dP1 As Double = Vk2(i) * (-(1 - sqTr) - (Vk3(i) - Tr) / (2 * sqTr))
                    Dim dP2 As Double = (0.7 - Tr) / (2 * sqTr) - (1 + sqTr)
                    dcidT = (dP1 * P2 + P1 * dP2) / Tc
                Else
                    If w <= 0.491 Then
                        ci = 0.37464 + 1.5422 * w - 0.26992 * w ^ 2
                    Else
                        ci = 0.379642 + 1.48503 * w - 0.164423 * w ^ 2 + 0.016666 * w ^ 3
                    End If
                    dcidT = 0.0
                End If
                Dim sfac As Double = 1 + ci * (1 - sqTr)
                ai(i) = 0.45724 * (sfac * sfac) * R ^ 2 * Tc ^ 2 / VPc(i)
                bi(i) = 0.0778 * R * Tc / VPc(i)
                Dim dsdT As Double = dcidT * (1 - sqTr) - ci / (2 * Math.Sqrt(T * Tc))
                daidT(i) = 2 * ai(i) * dsdT / sfac
            Next

            Dim a(n, n), dadT(n, n) As Double
            For i = 0 To n
                For j = 0 To n
                    Dim term As Double = VKij(i, j) * VKij2(j, i) / (Vx(i) * VKij(i, j) + Vx(j) * VKij2(j, i))
                    If Double.IsNaN(term) Or Double.IsInfinity(term) Then term = 0.0#
                    a(i, j) = (ai(i) * ai(j)) ^ 0.5 * (1 - term)
                    dadT(i, j) = a(i, j) * 0.5 * (daidT(i) / ai(i) + daidT(j) / ai(j))
                Next
            Next

            Dim sum1(n), sum2(n), sum3(n) As Double
            Dim dsum1(n), dsum2(n), dsum3(n) As Double
            For i = 0 To n
                For j = 0 To n
                    If i <> j Then
                        Dim sq As Double = (a(i, i) * a(j, j)) ^ 0.5
                        Dim dsq As Double = sq * 0.5 * (daidT(i) / ai(i) + daidT(j) / ai(j))
                        Dim f2 As Double = Vx(i) * Vx(j) * VKij(i, j) * VKij2(j, i) * ((1 - Vx(i)) * VKij(i, j) - Vx(j) * VKij2(j, i)) / (Vx(i) * VKij(i, j) + Vx(j) * VKij(j, i)) ^ 2
                        If Not Double.IsNaN(f2 * sq) Then
                            sum2(i) += f2 * sq
                            dsum2(i) += f2 * dsq
                        End If
                    End If
                    For k = 0 To n
                        If i <> j And k > j And k <> i Then
                            Dim sq As Double = (-a(j, j) * a(k, k)) ^ 0.5
                            Dim dsq As Double = sq * 0.5 * (daidT(j) / ai(j) + daidT(k) / ai(k))
                            Dim f3 As Double = Vx(j) * Vx(k) * (VKij(j, k) * VKij2(k, j)) / (Vx(j) * VKij(j, k) + Vx(k) * VKij2(k, j))
                            sum3(i) += f3 * sq
                            dsum3(i) += f3 * dsq
                        End If
                    Next
                    sum1(i) += Vx(j) * a(i, j)
                    dsum1(i) += Vx(j) * dadT(i, j)
                Next
            Next

            Dim aml As Double = 0.0, damldT As Double = 0.0, bml As Double = 0.0
            Dim aml2(n), daml2dT(n) As Double
            For i = 0 To n
                bml += Vx(i) * bi(i)
                For j = 0 To n
                    aml += Vx(i) * Vx(j) * a(i, j)
                    damldT += Vx(i) * Vx(j) * dadT(i, j)
                Next
                ' CalcLnFug drops sum3 when it comes out NaN, and the derivative has
                ' to drop it in exactly the same cases or it differentiates a
                ' different function.
                If Not Double.IsNaN(sum3(i)) Then
                    aml2(i) = sum1(i) + sum2(i) + sum3(i)
                    daml2dT(i) = dsum1(i) + dsum2(i) + dsum3(i)
                Else
                    aml2(i) = sum1(i) + sum2(i)
                    daml2dT(i) = dsum1(i) + dsum2(i)
                End If
            Next

            If aml <= 0.0 Or bml <= 0.0 Then Return Nothing

            Dim AG As Double = aml * P / (R * T) ^ 2
            Dim BG As Double = bml * P / (R * T)
            Dim dAGdT As Double = AG * (damldT / aml - 2.0 / T)
            Dim dBGdT As Double = -BG / T

            Dim zlist = ThermoPlugs.PR.CalcZ2(AG, BG)
            If zlist Is Nothing OrElse zlist.Count = 0 Then Return Nothing
            Dim Z As Double = If(TIPO = "L", zlist.Min, zlist.Max)

            Dim ZP As Double() = ThermoPlugs.PR.CheckRoot(Z, aml, bml, P, T, TIPO)
            If ZP Is Nothing OrElse Math.Abs(ZP(1) - P) > 0.00000001 * Math.Max(P, 1.0) Then Return Nothing
            Z = ZP(0)

            Dim c2 As Double = BG - 1.0
            Dim c1 As Double = AG - 3 * BG ^ 2 - 2 * BG
            Dim denom As Double = 3 * Z ^ 2 + 2 * c2 * Z + c1
            If Math.Abs(denom) < 1.0E-30 Then Return Nothing

            Dim dc2 As Double = dBGdT
            Dim dc1 As Double = dAGdT - 6 * BG * dBGdT - 2 * dBGdT
            Dim dc0 As Double = -(dAGdT * BG + AG * dBGdT) + 2 * BG * dBGdT + 3 * BG ^ 2 * dBGdT
            Dim dZdT As Double = -(Z ^ 2 * dc2 + Z * dc1 + dc0) / denom

            Dim e1 As Double = 1 + 2 ^ 0.5, e2 As Double = 1 - 2 ^ 0.5
            Dim num1 As Double = Z + e1 * BG, num2 As Double = Z + e2 * BG
            If num1 <= 0.0 Or num2 <= 0.0 Or Z <= BG Then Return Nothing
            Dim Lg As Double = Math.Log(num1 / num2)
            Dim dLg As Double = (dZdT + e1 * dBGdT) / num1 - (dZdT + e2 * dBGdT) / num2

            Dim deriv(n) As Double
            For i = 0 To n
                Dim Q As Double = AG / ((e1 - e2) * BG)
                Dim dQ As Double = Q * (dAGdT / AG - dBGdT / BG)
                Dim Gi As Double = 2 * aml2(i) / aml - bi(i) / bml
                Dim dGi As Double = 2 * (daml2dT(i) * aml - aml2(i) * damldT) / (aml * aml)
                Dim dT1 As Double = bi(i) / bml * dZdT
                Dim dT2 As Double = -(dZdT - dBGdT) / (Z - BG)
