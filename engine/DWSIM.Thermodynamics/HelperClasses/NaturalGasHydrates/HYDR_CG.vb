...
Namespace NaturalGasHydrates
    ...
    Public Class ChenGuo
        ...
        Function F0Pconst(ByVal id As Integer, ByVal hydrateStructure As String)
        ...
        Function AIJ_CG(ByVal i As Integer, ByVal j As Integer) As Double

            ' Chen & Guo (1998) binary interaction parameters Aij (K), structure II.
            ' Reference: Chen, G.-J.; Guo, T.-M. "A new approach to gas hydrate modelling",
            ' Chem. Eng. J. 71 (1998) 145-151, Table 3.
            ' Component IDs: 1=CH4, 2=C2H6, 3=C3H8, 4=iC4H10, 5=nC4H10, 14=H2S, 15=CO2, 16=N2.
            ' Matrix is symmetric: Aij = Aji.

            Dim ii As Integer, jj As Integer
            ' Order so that ii is the row index (CH4/N2/CO2/H2S) and jj is the column (C2H6/C3H8/iC4/nC4)
            If (i = 1 Or i = 14 Or i = 15 Or i = 16) AndAlso (j = 2 Or j = 3 Or j = 4 Or j = 5) Then
                ii = i : jj = j
            ElseIf (j = 1 Or j = 14 Or j = 15 Or j = 16) AndAlso (i = 2 Or i = 3 Or i = 4 Or i = 5) Then
                ii = j : jj = i
            Else
                Return 0.0
            End If
            ...
            Select Case ii
                Case 1   ' CH4
                    Select Case jj
                        Case 2 : Return 154.0   ' CH4-C2H6
                        Case 3 : Return 292.0   ' CH4-C3H8
                        Case 4 : Return 530.0   ' CH4-iC4H10
                        Case 5 : Return 100.0   ' CH4-nC4H10
                    End Select
                Case 16  ' N2
                    Select Case jj
                        Case 2 : Return 50.0    ' N2-C2H6
                        Case 3 : Return 155.0   ' N2-C3H8
                        Case 4 : Return 297.0   ' N2-iC4H10
                        Case 5 : Return 67.0    ' N2-nC4H10
                    End Select
                Case 15  ' CO2
                    Select Case jj
                        Case 2 : Return 165.0   ' CO2-C2H6
                        Case 3 : Return 352.0   ' CO2-C3H8
                        Case 4 : Return 560.0   ' CO2-iC4H10
                        Case 5 : Return 100.0   ' CO2-nC4H10
                    End Select
                Case 14  ' H2S
                    Select Case jj
                        Case 2 : Return 450.0   ' H2S-C2H6
                        Case 3 : Return 790.0   ' H2S-C3H8
                        Case 4 : Return 1500.0  ' H2S-iC4H10
                        Case 5 : Return 879.0   ' H2S-nC4H10
                    End Select
            End Select
        ...
        Function OBJ_FUNC_HYD_CG(ByVal hydrateType As String, ByVal P As Double, ByVal T As Double, ByVal Vz As Object, ByVal Vids As Object, Optional ByVal vaporonly As Boolean = False) As Object
            ...
            Do
                If i <> pos And Vids(i) <> 100 Then
                    Vx(i) = PHIV(i) / (H(i) * Math.Exp(ZLinf(i)))
            ...
            Do
                ...
                i = i + 1
            Loop Until i = n + 1
            ' Lump water and any inhibitors proportionally to fill the remainder of the aqueous phase
            i = 0
            Do
                If Vids(i) = 100 Or Vids(i) = 13 Then
                    Vx(i) = Vz(i) / (1 - sum) * (1 - sum_vxaq)
                End If
                i = i + 1
            Loop Until i = n + 1
        ...
        Function HYD_CG2(ByVal T As Double, ByVal Vz As Object, ByVal Vids As Object, Optional ByVal vaporonly As Boolean = False) As Object
            ...
            If sI_formers = False Then hydrateType = "sII"
...
START_LOOP:
            ...
            Do
                fP = OBJ_FUNC_HYD_CG(hydrateType, Pinf, T, Vz, Vids, vaporonly)
                Pinf = Pinf + delta_P
                fP_inf = OBJ_FUNC_HYD_CG(hydrateType, Pinf, T, Vz, Vids, vaporonly)
            ...
            faa = OBJ_FUNC_HYD_CG(hydrateType, aaa, T, Vz, Vids, vaporonly)
            fbb = OBJ_FUNC_HYD_CG(hydrateType, bbb, T, Vz, Vids, vaporonly)
            fcc = fbb
...
STEP2:
            ...
            If sI_formers = True And sII_formers = True And hydrateType = "sI" Then
                hydrateType = "sII"
                PsI = P
                GoTo START_LOOP
            ElseIf sI_formers = True And sII_formers = True And hydrateType = "sII" Then
                PsII = P
            ElseIf sI_formers = False And sII_formers = True And hydrateType = "sI" Then
                hydrateType = "sII"
                GoTo START_LOOP
            ElseIf sI_formers = True And sII_formers = False And hydrateType = "sI" Then
                PsI = P
                PsII = P
...
START_LOOP:
            ...
            Do
                fT = OBJ_FUNC_HYD_CG(hydrateType, P, Tinf, Vz, Vids, vaporonly)
                Tinf = Tinf + delta_T
                fT_inf = OBJ_FUNC_HYD_CG(hydrateType, P, Tinf, Vz, Vids, vaporonly)
            ...
            faa = OBJ_FUNC_HYD_CG(hydrateType, P, aaa, Vz, Vids, vaporonly)
            fbb = OBJ_FUNC_HYD_CG(hydrateType, P, bbb, Vz, Vids, vaporonly)
            fcc = fbb
...
STEP2:
        ...
        Function DET_HYD_CG(ByVal hydrateType As String, ByVal P As Double, ByVal T As Double, ByVal Vz As Object, ByVal Vids As Object, Optional ByVal vaporonly As Boolean = False) As Object
